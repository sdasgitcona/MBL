...
@Directive({
  selector: '[appBillsPendingApproval]'
})
export class BillsPendingApprovalDirective {
  ...
  getApiData(token: any, subsidiaryId: any) {
    this.httpService
      .GetById(`/masters-ws/item/get-active-inActive-item-count?subsidiaryId=` + subsidiaryId, subsidiaryId, token)
      .subscribe((res) => {
        if (res.status == 401) {

...
@Component({
  selector: 'app-pr-aging',
  templateUrl: './pr-aging.component.html',
  styleUrls: ['./pr-aging.component.scss']
})
export class PrAgingComponent implements OnInit {
  ...
  initbarSort() {
    ...
    this.httpService.GetById(`/procure-ws/pr/get-pending-pr-count-day-wise?subsidiaryIds=` + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
      .subscribe(res => {
        let root = am5.Root.new("chartbarSortDaywise");
  ...
  toggleFullscreen() {
    let div = document.querySelector('.fullscreenDivMonthlyDayWise');

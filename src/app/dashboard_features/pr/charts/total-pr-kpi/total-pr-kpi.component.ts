...
@Component({
  selector: 'app-total-pr-kpi',
  templateUrl: './total-pr-kpi.component.html',
  styleUrls: ['./total-pr-kpi.component.scss']
})
export class TotalPrKPIComponent implements OnInit {
  ...
  initKPI() {
    ...
    bullet.get("sprite").on("rotation", function () {
      ...
      label.set("text", `Processed PR : ${Math.round(value).toString()}`);
      Total_label.set("text", `Total PR : ${Math.round(Total_value).toString()}`);
  ...
  toggleFullscreen() {
    let div = document.querySelector('.fullscreenDivTotalPr');

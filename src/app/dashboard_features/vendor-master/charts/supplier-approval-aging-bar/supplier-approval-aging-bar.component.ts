...
@Component({
  selector: 'app-supplier-approval-aging-bar',
  templateUrl: './supplier-approval-aging-bar.component.html',
  styleUrls: ['./supplier-approval-aging-bar.component.scss']
})
export class SupplierApprovalAgingBarComponent implements OnInit {
    ...
    this.httpService.GetById(`/finance-ws/invoice/get-approval-aging-invoice?subsidiaryId=` + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
      .subscribe(res => {
        ...
        this.chartData = [
          ...
          {
              "category": "14+ Days Payment",
              "value": 6
          },
          {
              "category": "21+ days Payment",
              "value": 8
          }
      ]
  ...
  getChart() {
    this.httpService.GetById(`/finance-ws/invoice/get-approval-aging-invoice?subsidiaryId=` + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
      .subscribe(res => {
        ...
        let root = am5.Root.new("SupplierApprovalAging");
  ...
  toggleFullscreen() {
    let div = document.querySelector('.fullscreenDivMonthlyDayWiseSupplierApprovalAging');

...
@Component({
  selector: 'app-total-received-total-return-grn',
  templateUrl: './total-received-total-return-grn.component.html',
  styleUrls: ['./total-received-total-return-grn.component.scss']
})
export class TotalReceivedTotalReturnGrnComponent implements OnInit {
  ...
  getApiData(){
    this.httpService
    .GetById('/procure-ws/grn/get-status-wise-count?subsidiaryId=' + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
    .subscribe((res) => {
      if (res.status == 401) {

...
@Directive({
  selector: '[appPartiallyReceivedPo]'
})
export class PartiallyReceivedPoDirective {
  ...
  getApiData(token: any, subsidiaryId: any) {
    this.httpService
    .GetById(`/procure-ws/grn/partially-received-grn?subsidiaryId=` + subsidiaryId, subsidiaryId, token)
      .subscribe((res) => {
        if (res.status == 401) {
          ...
          if (this.data && this.data.length > 0) {
            this.Cards.push({
              ...
              category: 'Partially Received PO',
              link: '/main/purchase-order/list',
              param:'Partially Received',

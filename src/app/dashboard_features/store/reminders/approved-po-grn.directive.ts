...
@Directive({
  selector: '[appApprovedPoGrn]'
})
export class ApprovedPoGrnDirective {
  ...
  getApiData(token: any, subsidiaryId: any) {
    this.httpService
    .GetById(`/procure-ws/po/get-billed-approved-po?subsidiaryId=` + subsidiaryId, subsidiaryId, token)
      .subscribe((res) => {
        if (res.status == 401) {

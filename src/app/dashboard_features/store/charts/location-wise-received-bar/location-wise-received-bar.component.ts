...
@Component({
  selector: 'app-location-wise-received-bar',
  templateUrl: './location-wise-received-bar.component.html',
  styleUrls: ['./location-wise-received-bar.component.scss']
})
export class LocationWiseReceivedBarComponent implements OnInit {
  ...
  initbarSort() {
    this.httpService.GetById(`/procure-ws/grn/location-wise-received-grn?subsidiaryId=` + this.selectedSubsidiaryId + `&fromDate=` + this.fromDate + `&toDate=` + this.toDate, this.selectedSubsidiaryId, this.RetloginDetails.token)
      .subscribe(res => {
        this.chartData = res;

        let root = am5.Root.new("chartbarSortLocationWiseReceivedGRN");
  ...
  toggleFullscreen() {
    let div = document.querySelector('.fullscreenDivLocationWiseReceivedGRN');

...
@Component({
  selector: 'app-grn-vs-bill-kpi',
  templateUrl: './grn-vs-bill-kpi.component.html',
  styleUrls: ['./grn-vs-bill-kpi.component.scss']
})
export class GrnVsBillKpiComponent implements OnInit {
  ...
  initKPI() {
    this.httpService.GetById(`/procure-ws/grn/total-grn-and-billed-grn?subsidiaryId=` + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
      .subscribe(res => {
        this.min = res[0].value;
        this.max = res[1].value;

        let root = am5.Root.new("GrnVsBillKpi");
        ...
        bullet.get("sprite").on("rotation", function () {
          ...
          label.set("text", `Billed GRN : ${Math.round(value).toString()}`);
          Total_label.set("text", `Total GRN : ${Math.round(Total_value).toString()}`);
  ...
  toggleFullscreen() {
    let div = document.querySelector('.fullscreenDivGrnVsBill');

...
@Directive({
  selector: '[appAwatingForApprovalAp]'
})
export class AwatingForApprovalApDirective {
  ...
  getApiData(employeeId: any,token: any, subsidiaryId: any) {
    this.httpService
    .GetById(`/finance-ws/invoice/approval-list-count?empId=${employeeId}&subsidiaryId=${subsidiaryId}`,subsidiaryId,token)
      .subscribe((res) => {
        if (res.status == 401) {
          ...
          if (this.data) {
            this.Cards.push({
              ...
              category: 'Awating for Approval AP Invoice',
              link: '/main/apinvoice/APlist',
              param:'Approval',

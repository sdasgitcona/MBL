...
@Component({
  selector: 'app-total-open-ap',
  templateUrl: './total-open-ap.component.html',
  styleUrls: ['./total-open-ap.component.scss']
})
export class TotalOpenApComponent implements OnInit {
  ...
  getApiData() {
    this.httpService
      .GetById(`/finance-ws/advance/get-open-and-total-count-for-advanced-payment?subsidiaryId=` + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
      .subscribe((res) => {
        if (res.status == 401) {

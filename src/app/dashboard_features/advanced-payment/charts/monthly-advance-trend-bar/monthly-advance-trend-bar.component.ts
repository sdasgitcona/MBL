...
@Component({
  selector: 'app-monthly-advance-trend-bar',
  templateUrl: './monthly-advance-trend-bar.component.html',
  styleUrls: ['./monthly-advance-trend-bar.component.scss']
})
export class MonthlyAdvanceTrendBarComponent implements OnInit {
  ...
  initbarSort() {
    ...
    this.httpService.GetById(`/finance-ws/advance/get-pending-aging-advanced-payment?subsidiaryId=` + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
      .subscribe(res => {
        let root = am5.Root.new("AdvPayApprovalAging");
  ...
  toggleFullscreenMonthelyAdvanceTrend() {
    let div = document.querySelector('.fullscreenDivAdvPayApprovalAging');

...
@Component({
  selector: 'app-total-vs-approved-payment',
  templateUrl: './total-vs-approved-payment.component.html',
  styleUrls: ['./total-vs-approved-payment.component.scss']
})
export class TotalVsApprovedPaymentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.httpService.GetById(`/finance-ws/advance/approved-and-paid-for-advanced-payement?subsidiaryId=` + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
    .subscribe(res => {
      this.min = res[1].value;
      this.max = res[0].value;
  ...
  initKPI() {
    ...
    let root = am5.Root.new("totalvsApprovedPayKpi");
    ...
    bullet.get("sprite").on("rotation", function () {
      ...
      Total_label.set("text", `Approved Bill : ${Math.round(Total_value).toString()}`);
  ...
  toggleFullscreen() {
    let div = document.querySelector('.fullscreenDivTotalvsApprovedPay');

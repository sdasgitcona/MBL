...
@Component({
  selector: 'app-total-supplier',
  templateUrl: './total-supplier.component.html',
  styleUrls: ['./total-supplier.component.scss']
})
export class TotalSupplierComponent implements OnInit {
  ...
  getApiData(){
    this.httpService
    .GetById('/procure-ws/po/get-active-and-total-supplier-count?subsidiaryId=' + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
    .subscribe((res) => {
      if (res.status == 401) {

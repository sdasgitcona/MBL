...
@Component({
  selector: 'app-total-created-recentely-created-company',
  templateUrl: './total-created-recentely-created-company.component.html',
  styleUrls: ['./total-created-recentely-created-company.component.scss']
})
export class TotalCreatedRecentelyCreatedCompanyComponent implements OnInit {
  ...
  getApiData(){
    this.httpService
    .GetById('/setup-ws/company-data/get-company-count', this.selectedSubsidiaryId, this.RetloginDetails.token)
    .subscribe((res) => {
      if (res.status == 401) {

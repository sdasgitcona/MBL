...
@Component({
  selector: 'app-payment-approval-aging',
  templateUrl: './payment-approval-aging.component.html',
  styleUrls: ['./payment-approval-aging.component.scss']
})
export class PaymentApprovalAgingComponent implements OnInit {
  ...
  initbarSort() {
    ...
    this.httpService.GetById(`/finance-ws/payment/get-approval-aging-make-payment?subsidiaryId=` + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
      .subscribe(res => {
        let root = am5.Root.new("chartbarPaymentApprovalAge");
  ...
  toggleFullscreen() {
    let div = document.querySelector('.fullscreenDivPaymentApprovalAge');

...
@Directive({
  selector: '[appAwatingForApprovalRtv]'
})
export class AwatingForApprovalRtvDirective {
  ...
  getApiData(employeeId: any,token: any, subsidiaryId: any) {
    this.httpService
    .GetById(`/procure-ws/rtv/pending-partial-count?empId=${employeeId}&subsidiaryId=${subsidiaryId}`,subsidiaryId,token)
      .subscribe((res) => {
        if (res.status == 401) {
          ...
          if (this.data) {
            this.Cards.push({
              ...
              category: 'Awating for Approval RTV',
              link: '/main/rtv/rtv-approval',
              param:'Approval',

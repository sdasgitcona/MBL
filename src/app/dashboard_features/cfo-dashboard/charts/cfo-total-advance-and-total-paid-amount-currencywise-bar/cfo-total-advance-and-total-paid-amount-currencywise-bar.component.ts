...
@Component({
  selector: 'app-cfo-total-advance-and-total-paid-amount-currencywise-bar',
  templateUrl: './cfo-total-advance-and-total-paid-amount-currencywise-bar.component.html',
  styleUrls: ['./cfo-total-advance-and-total-paid-amount-currencywise-bar.component.scss']
})
export class CfoTotalAdvanceAndTotalPaidAmountCurrencywiseBarComponent implements OnInit {
  ...
  initbarSort() {
    ...
    this.httpService.GetById(`/finance-ws/advance/get-total-advance-payment-currency-wise?subsidiaryIds=`+this.selectedSubsidiaryId , this.selectedSubsidiaryId, this.RetloginDetails.token)
      .subscribe(res => {
        ...
        let root = am5.Root.new("TotalAdvanceAndTotalPaid");
        ...
        this.makeSeries("Total Amount", "totalAmount");
        this.makeSeries("Paid Amount", "totalPaidAmount");
  ...
  toggleFullscreen() {
    let div = document.querySelector('.fullscreenDivProjectWiseTotalAdvanceAndTotalPaid');

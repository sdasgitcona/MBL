...
@Component({
  selector: 'app-cfo-total-po-and-unapproved',
  templateUrl: './cfo-total-po-and-unapproved.component.html',
  styleUrls: ['./cfo-total-po-and-unapproved.component.scss']
})
export class CfoTotalPoAndUnapprovedComponent implements OnInit {
  ...
  getApiData() {
    ...
    this.httpService
      .GetById(`/procure-ws/po/get-total-po-and-unapproved?subsidiaryIds=` + this.selectedSubsidiaryId, this.selectedSubsidiaryId, this.RetloginDetails.token)
      .subscribe((res) => {
        if (res.status == 401) {
